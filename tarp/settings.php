...
if (isset($_REQUEST['uid'])) {
	...
	$user2 = mysqli_real_escape_string($connection,$_REQUEST['uid']);
	if($user != $user2){
		...
		if(empty($_POST['email'])) {
			...
			if(isset($opass) && isset($npass) && isset($npass1) && ($opass != "" && $npass != "" && $npass1 != "")){
				if( md5($opass) == $upass){
					if($npass == $npass1){
						$npass = md5($npass);
						mysqli_query($connection,"UPDATE user SET password='$npass' WHERE id='$user'");
						$success_message = '
			...
			if($uemail_db != $email) {
				if(mysqli_query($connection,"UPDATE user SET  email='$email' WHERE id='$user'")){
					//success message
					$success_message = '
					<div class="signupform_text" style="font-size: 18px; text-align: center;">
					<font face="bookman">
