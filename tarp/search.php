...
if (!isset($_SESSION['user_login'])) {
	...
	$aVar = mysqli_connect('localhost','root','','ebuybd');
	$result = mysqli_query($aVar, "SELECT * FROM user WHERE id='$user'");
...
if (isset($_REQUEST['keywords'])) {
	...
	$epid = mysqli_real_escape_string($connection,$_REQUEST['keywords']);
	...
	<div style="padding: 30px 120px; font-size: 25px; margin: 0 auto; display: table; width: 98%;">
		...
		<?php 
			if (isset($_GET['keywords']) && $_GET['keywords'] != ""){
				$search_value = trim($_GET['keywords']);
				$getposts = mysqli_query($connection,"SELECT * FROM products WHERE pName like '%$search_value%'  ORDER BY id DESC") or die(mysql_error());
