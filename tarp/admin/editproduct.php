...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['epid'])) {
		...
		$epid = mysqli_real_escape_string($connection,$_REQUEST['epid']);
	}else {
...
$getposts = mysqli_query($connection,"SELECT * FROM products WHERE id ='$epid'") or die(mysqli_error());
...
if (isset($_POST['updatepro'])) {
	...
	if($result = mysqli_query($connection,"UPDATE products SET pName='$_POST[pname]',price='$_POST[price]',description='$_POST[descri]',available='$_POST[available]',category='$_POST[category]',type='$_POST[type]',item='$_POST[item]',pCode='$_POST[code]' WHERE id='$epid'")){
...
//triming name
	$getposts1 = mysqli_query($connection,"SELECT pid FROM orders WHERE pid='$epid'") or die(mysqli_error());
					if ($ttl = mysqli_num_rows($getposts1)) {
						$error_message = "You can not delete this product.<br>Someone ordered this.";
					}
					else {
						if(mysqli_query($connection,"DELETE FROM products WHERE id='$epid'")){
	...
	<div class="homepageheader">
			...
			<div id="srcheader">
				<form id="newsearch" method="get" action="search.php">
				        <?php 
				        	echo '<inpu type="text" class="srctextinput" name="keywords" size="21" maxlength="120"  placeholder="Search Here..." value="'.$search_value.'"><input type="submit" value="search" class="srcbutton" >';
				         ?>
				</form>
