...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$eoid = mysqli_real_escape_string($connection,$_REQUEST['eoid']);
		$getposts5 = mysqli_query($connection,"SELECT * FROM orders WHERE id='$eoid'") or die(mysql_error());
	...
	$result1 = mysqli_query($connection,"SELECT * FROM orders WHERE id='$eoid'");
		$get_order_info = mysqli_fetch_assoc($result1);
			...
			$result2 = mysqli_query($connection,"SELECT * FROM user WHERE id='$eouid'");
...
$getposts = mysqli_query($connection,"SELECT * FROM products WHERE id ='$eopid'") or die(mysql_error());
	...
	try {
		if(empty($_POST['dstatus'])) {
			throw new Exception('Status can not be empty');
				...
				if(mysqli_query($connection,"UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")){
					//success message
...
//triming name
	if(mysqli_query($connection,"DELETE FROM orders WHERE id='$eoid'")){
